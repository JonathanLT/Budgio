...
export function GoalsPanel({ householdId, token }: Props) {
  ...
  function handleDrop(targetId: string) {
    ...
    setGoals((prev) => {
      ...
      const items = next.map((g, i) => ({ id: g.id, order: i }));
      void api.reorderGoals(token, householdId, items);
      return next;
    });
  }
      ...
      {completed.length > 0 && (
        <div className="space-y-3">
          <button
            ...
            className="flex items-center gap-2 text-sm font-medium text-theme-muted hover:text-theme-text transition-colors w-full text-left"
          >
...
function GoalCard({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      onDragEnd={onDragEnd}
      className={`bg-theme-surface rounded-xl border border-theme-border p-5 group transition-all ${goal.isCompleted ? 'opacity-70' : ''} ${dragging ? 'ring-2 ring-brand-500/40' : ''}`}
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2">
          {onDragStart && <span className="text-theme-muted cursor-grab active:cursor-grabbing select-none opacity-0 group-hover:opacity-100 transition-opacity" title="Réordonner">⠿</span>}
